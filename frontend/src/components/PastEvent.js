...
const AdminHeader = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const PastdisplayedEntries = responseData.filter(event => new Date(event.endDate) < new Date());
            setPastEventsEnteries(PastdisplayedEntries);
            setFilteredEntries(PastdisplayedEntries);
        } catch (error) {
    ...
    const handleReportClick = () => {
        ...
        const worksheet = XLSX.utils.json_to_sheet(PastEventsEnteries);
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Past Event Report');
        const timestamp = Date.now();
        XLSX.writeFile(workbook, `admin_past_event_report_${timestamp}.xlsx`);
    };
